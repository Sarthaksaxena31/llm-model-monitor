...
export class CheckerService {
    ...
    async handleAuthFailure(provider) {
        const db = await getDb();
        await db.run('UPDATE model_registry SET status = ?, failureCount = failureCount + 1 WHERE provider = ?', ['auth_failure', provider]);
    ...
    async handleDeprecation(model) {
        const db = await getDb();
        if (model.status !== 'deprecated') {
            await db.run('UPDATE model_registry SET status = "deprecated", lastVerified = ? WHERE id = ?', [new Date().toISOString(), model.id]);
            await this.alerter.sendAlert({
    ...
    async handleSuccess(model) {
        const db = await getDb();
        await db.run('UPDATE model_registry SET status = "active", failureCount = 0, lastError = NULL, lastVerified = ? WHERE id = ?', [new Date().toISOString(), model.id]);
    }
    async handleGeneralError(provider, status, error) {
        const db = await getDb();
