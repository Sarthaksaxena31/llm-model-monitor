...
import { type ModelRegistry } from '../types/index.js';
...
async function seedModels() {
    ...
    for (const model of initialModels) {
        ...
        if (!existing) {
            await database.run(
                'INSERT INTO model_registry (provider, modelId, status, lastVerified, metadata) VALUES (?, ?, ?, ?, ?)',
                [model.provider, model.modelId, model.status, new Date().toISOString(), model.metadata]
            );
        }
    }
}
